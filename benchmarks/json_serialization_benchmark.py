...
try:
    ...
    HAS_OPTIMIZED = False
    print("⚠️  Optimized JSON responses not available. "
...
def run_benchmarks():
    ...
    for size in test_sizes:
        ...
        if HAS_OPTIMIZED:
            opt_results = benchmark_optimized_json(data, iterations=10, parallel_threshold=50)
    ...
    if HAS_OPTIMIZED:
        ...
        for threshold in thresholds:
            ...
            results = benchmark_optimized_json(large_data, iterations=5, parallel_threshold=threshold)
...
async def async_benchmark():
    ...
    for data in datasets:
        task = asyncio.create_task(asyncio.to_thread(
            lambda d: StandardJSONResponse(d).render(d), data
        ))
        tasks.append(task)
    ...
    for data in datasets:
        task = asyncio.create_task(asyncio.to_thread(
            lambda d: OptimizedJSONResponse(d).render(d), data
        ))
        tasks.append(task)
