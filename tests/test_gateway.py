...
class MockScope:
    ...
    def __init__(self, proto="http", method="GET", path="/", headers=None, query_string=""):
        self.proto = proto
        self.method = method
        self.path = path
...
class TestGatewayIntegration:
    ...
    async def test_forward_to_function(self):
        ...
        with patch('velithon.gateway.ProxyClient') as mock_proxy_client:
            ...
            mock_client.forward_request = AsyncMock(return_value=(
                200, b"test response", {"Content-Type": "application/json"}
            ))
