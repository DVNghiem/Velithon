...
from velithon.params.params import Body, File, Form, Header, Path, Query, Cookie
...
class DocumentationConfig:
    ...
    def __init__(
        ...
        self.servers = servers or [{"url": "http://localhost:8000", "description": "Development server"}]
        self.include_examples = include_examples
        self.include_schemas = include_schemas
        self.group_by_tags = group_by_tags
        self.exclude_routes = set(exclude_routes or [])
        self.include_only_routes = set(include_only_routes or []) if include_only_routes else None
...
class DocumentationGenerator:
    ...
    def _python_type_to_api_type(self, annotation: Any) -> str:
        ...
        for pattern, api_type in name_mappings.items():
            ...
            if pattern == 'Request' and type_name == 'Request':
                ...
                return api_type
            elif (pattern in ['datetime', 'UUID', 'Decimal', 'Path', 
                             'AnyUrl', 'EmailStr', 'Json'] and 
                  pattern.lower() in type_name.lower()):
                return api_type
            elif pattern == 'date' and (type_name == 'date' or type_name.endswith('.date')):
                return api_type
            elif pattern == 'time' and (type_name == 'time' or type_name.endswith('.time')):
                return api_type
            elif pattern in ['UserStatus', 'Priority'] and pattern in type_name:
                return api_type
        ...
        is_union = (origin is Union or 
                   str(type(annotation)).startswith("<class 'types.UnionType'>"))
        if is_union:
    ...
    def _extract_parameter_info(self, func: Any, route_path: str = "") -> list[ParameterInfo]:
        ...
        for param_name, param in sig.parameters.items():
            ...
            # Determine parameter type and get description
            ...
            if param.default != inspect.Parameter.empty:
                ...
                if isinstance(param.default, Path):
                    ...
                    param_constraint = param.default
            ...
            if get_origin(param.annotation) is Annotated:
                ...
                for metadata in args[1:]:
                    if isinstance(metadata, (Path, Query, Body, Header, Cookie, Form, File)):
            ...
            if self._is_pydantic_model(actual_type):
                ...
                if model_fields:
                    ...
                    detailed_description = f"**{type_name} model with fields:**\n" + "\n".join(field_descriptions)
                    
                    # For Pydantic models, create a clean table description and detailed description
                    if param_info.description and param_info.description != detailed_description:
    ...
    def _extract_docstring_info(self, func: Any) -> tuple[str, str, list[dict[str, str]]]:
        ...
        for line in lines[1:]:
            ...
            if line.lower().startswith(('args:', 'arguments:', 'parameters:', 'param:')):
                ...
                continue
            elif line.lower().startswith(('raises:', 'raise:', 'examples:', 'example:')):
                in_params_section = False
    ...
    def _should_include_route(self, route_path: str, route_name: str) -> bool:
        ...
        # Check exclude list
        if route_path in self.config.exclude_routes or route_name in self.config.exclude_routes:
            return False

        # Check include only list
        if self.config.include_only_routes:
            return (route_path in self.config.include_only_routes or
                   route_name in self.config.include_only_routes)
        ...
        def extract_from_routes(routes):
            for route in routes:
                if hasattr(route, 'endpoint') and route.endpoint:
                    route_name = getattr(route.endpoint, '__name__', str(route.endpoint))
